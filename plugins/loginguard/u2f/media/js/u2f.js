 ...
 *
 * @param   regData  string  The JSON-serialized U2F registration request generated by the server
 */
akeeba.LoginGuard.u2f.setUp = function (regData)
{
	var u2fResponse      = JSON.parse(regData);
	var u2fRequest       = u2fResponse[0];
	var u2fAuthorization = u2fResponse[1];
...
akeeba.LoginGuard.u2f.validate = function (authDataEncoded)
{
	var authData = JSON.parse(authDataEncoded);

	u2f.sign(authData, function (response)
	{
