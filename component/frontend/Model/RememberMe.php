...
class RememberMe extends Model
{
	...
	public function getCookieName(): ?string
	{
		...
		$isEnabled = (bool) $this->container->params->get(self::CONFIG_KEY_ENABLED, true);
	...
	public function setCookie(): void
	{
		...
		$lifetime    = $this->container->params->get(self::CONFIG_KEY_LIFETIME, 15) * 24 * 3600;
		$expiration  = time() + $lifetime;
		$length      = $this->container->params->get(self::CONFIG_KEY_KEYLENGTH, 64);
	...
	public function getBrowserId(): string
	{
		...
		{
			$browserId = $this->container->session->get('browserId', null, 'com_loginguard');
	 ...
	 * @since   3.3.0
	 */
	public function setBrowserId(string $browserId): RememberMe
	{
	 ...
	 * @since   3.3.0
	 */
	public function setUsername(string $username): RememberMe
	{
	...
	private function storeToken(string $series, string $token, int $expiration): void
	{
		...
		$query  = $db->getQuery(true)
			...
			->where($db->qn('user_id') . ' = ' . $db->q($username))
			->where($db->qn('series') . ' = ' . $db->q($series))
			->where($db->qn('uastring') . ' = ' . $db->q($cookieName));
